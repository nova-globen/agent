...
.\install.ps1            # or: .\install.ps1 -Version v0.2.0
```
...
```json
{
  ...
  "isRoot": true,
  "tools": {
    "agentsync": { "version": "0.2.0", "commands": ["agent"] },
    "agentsync.git": { "version": "0.2.0", "commands": ["git-agent"] }
  }
}
```
...
2. Download the archive for your OS/architecture, e.g.
   `agent-sync-v0.2.0-linux-x64.tar.gz` (or `...-win-x64.zip` on Windows).
3. Extract it.
